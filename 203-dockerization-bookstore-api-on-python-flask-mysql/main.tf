...
resource "aws_instance" "tf-docker-ec2" {
  ...
  key_name        = "first_key"
  security_groups = ["${aws_security_group.tf-docker-ec2-sec-gr.name}"]
  tags = {
  ...
  user_data  = <<-EOF
          ...
          FOLDER="https://$TOKEN@raw.githubusercontent.com/KeremKaragulle/bookstore-repo/main/"
...
output "website" {
  value = "http://${aws_instance.tf-docker-ec2.public_ip}:80"
}